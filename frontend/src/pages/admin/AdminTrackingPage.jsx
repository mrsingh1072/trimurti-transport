...
/**
 * Admin Live Vehicle Tracking Page - SaaS Grade UI
 * Premium design with system stats and:
        ...
        <div className={`sidebar-desktop md:relative w-full md:w-80 h-full bg-white flex flex-col border-r border-gray-200 z-40 ${mobileOpen ? 'open' : ''}`}>
          ...
          <div className="p-4 border-b border-gray-200 flex-shrink-0">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Search vehicles..." value={searchLocation} onChange={(e) => setSearchLocation(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent bg-white transition-all" />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {loading && vehicles.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40">
                <div className="w-8 h-8 border-3 border-purple-200 border-t-purple-500 rounded-full animate-spin mb-3"></div>
          ...
          <div className="p-4 border-t border-gray-200 flex-shrink-0">
            <div className="bg-gradient-to-br from-purple-500 to-cyan-500 rounded-2xl p-4 text-white">
              <p className="text-xs font-bold tracking-widest mb-2">SYSTEM OVERVIEW</p>
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <p className="font-bold text-lg">{vehicles.length}</p>
                  <p className="text-purple-100">Total Vehicles</p>
                </div>
                <div>
                  <p className="font-bold text-lg">{searchedVehicles.length}</p>
                  <p className="text-purple-100">Now Tracked</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`fixed bottom-0 left-0 right-0 md:left-80 bg-white rounded-t-3xl shadow-2xl z-50 bottom-sheet ${mobileOpen ? 'hidden md:block' : ''}`} style={{ height: '160px' }}>
            ...
            <div className="px-6 py-4 flex items-center justify-between gap-4 h-full">
              <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-cyan-500 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0 border-3 border-purple-300">
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button className="px-4 py-2 bg-gradient-to-r from-purple-500 to-cyan-500 text-white font-bold rounded-lg text-sm hover:shadow-lg hover:scale-105 transition-all">
                  COMPLETE
                </button>
                <button className="px-4 py-2 border-2 border-purple-500 text-purple-600 font-bold rounded-lg text-sm hover:bg-purple-50 transition-all">
                  REPORT
                </button>
              </div>
            </div>
